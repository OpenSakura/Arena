 ...
 *
 * Leaderboard page with improved visual polish.
 */
...
export default async function LeaderboardPage({
  ...
  const request = buildLeaderboardQuery(searchParams);

  let models: LeaderboardRow[] = [];
  let errorText: string | null = null;
  let selectedMethod: "elo" | "bt" = request.selectedMethod;
  let includeConfidence = request.includeConfidence;
  let bootstrapRounds: number | null = null;
  ...
  try {
    ...
    bootstrapRounds = data.bootstrap_rounds;
  } catch (err) {
    errorText = err instanceof Error ? err.message : "Failed to load leaderboard";
  }
    ...
    <div className="grid gap-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-1.5 rounded-xl border border-border/50 bg-background/30 p-1 backdrop-blur">
          ...
          <Link
            href={`/leaderboard?method=bt${includeConfidence ? "&include_confidence=true" : ""}`}
            className={`rounded-lg px-4 py-1.5 text-xs font-semibold transition-all ${
              selectedMethod === "bt"
                ? "bg-primary/10 text-primary shadow-sm"
                : "text-muted-foreground hover:bg-foreground/5 hover:text-foreground"
