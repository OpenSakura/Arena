...
requests must include the per-session CSRF token returned by
`GET /api/v1/auth/session` in the configured CSRF header, default
`X-CSRF-Token`, plus the HttpOnly session cookie set by login.
